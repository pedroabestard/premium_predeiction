...
scaler_total = joblib.load("artifacts/scaler.joblib")
...
def calculate_normalized_life_style_score(physical_activity,stress_level):
    risk_score_physical_activity = {
        'High': 0,
        'Medium': 1,
        'Low': 4
    }

    risk_score_stress_level = {
        'High': 4,
        'Medium': 1,
        'Low': 0
    }

    total_life_style_score = risk_score_physical_activity.get(physical_activity) + risk_score_stress_level.get(stress_level)
    min_score = 0
    max_score = 8
    normalized_life_style_score = (total_life_style_score - min_score) / (max_score - min_score)
    return normalized_life_style_score
...
def preprocess_input(input_dict):
    ...
    df['normalized_life_style_risk_score'] = calculate_normalized_life_style_score(input_dict['Physical Activity'],input_dict['Stress Level'])
    df = handle_scaling(df)

    return df

def handle_scaling(df):
    cols_to_scale = scaler_total['cols_to_scale']
    scaler = scaler_total['scaler']
